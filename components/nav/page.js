  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between relative">
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full right-6 mt-6 w-1/2 bg-zinc-950 border border-zinc-800 rounded-md shadow-xl z-40 py-6 flex flex-col gap-6">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => scrollToSection(item.id)}
                className={`px-6 text-left text-lg uppercase tracking-widest transition-colors ${
                  activeSection === item.id
            ...
            <div className="px-6 pt-2">
              <button
                onClick={() => scrollToSection("contact")}
                className="w-full px-4 py-2 border border-zinc-700 hover:border-white text-white text-xs uppercase tracking-widest rounded-sm transition-all hover:bg-white hover:text-zinc-950"
              >
                Consult Now
              </button>
            </div>
